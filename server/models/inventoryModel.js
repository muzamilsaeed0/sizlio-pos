...
async function getItemById(id, restaurantId) {
  const query = `
    SELECT
      ...
      CASE WHEN stock_quantity <= minimum_stock THEN true ELSE false END AS low_stock
    FROM inventory_items
...
async function setItemActive(id, restaurantId, isActive) {
  const query = `
    ...
    RETURNING *
  `;
  const result = await pool.query(query, [isActive, id, restaurantId]);
  return result.rows[0] || null;
}
...
async function addTransaction({
  ...
  try {
    ...
    if (itemResult.rows.length === 0) throw new Error("Inventory item not found");

    const item = itemResult.rows[0];
    if (!item.is_active) throw new Error("Inventory item is inactive");
    ...
    if (["in", "purchase", "add", "restock"].includes(normalizedType)) {
      ...
      transactionType = "IN";
    } else if (["out", "usage", "sale", "remove", "waste"].includes(normalizedType)) {
      newStock = currentStock - amount;
    ...
    const transactionResult = await client.query(
      ...
      [inventory_id, transactionType, amount, note]
    );

    const itemUpdate = await client.query(
      `UPDATE inventory_items SET stock_quantity = $1
       WHERE id = $2 AND restaurant_id = $3 RETURNING *`,
      [newStock, inventory_id, restaurant_id]
    );
...
async function getTransactions(inventoryId, restaurantId) {
  const query = `
    SELECT
      t.id, t.inventory_id, t.type, t.quantity, t.note, t.created_at,
      i.name AS inventory_name, i.unit
    FROM inventory_transactions t
    INNER JOIN inventory_items i ON i.id = t.inventory_id
    WHERE t.inventory_id = $1 AND i.restaurant_id = $2
    ORDER BY t.created_at DESC, t.id DESC
  `;

  const result = await pool.query(query, [inventoryId, restaurantId]);
  return result.rows;
}

// ======================================================
// LIST INVENTORY FOR PURCHASE (NEW)
// ======================================================

async function listInventoryForPurchase(restaurantId) {
  const query = `
    SELECT
      id,
      name,
...
async function getRecipe(menuItemId, restaurantId, variantId = null) {
  let query = `
    SELECT
      mii.id, mii.menu_item_id, mii.inventory_id, mii.variant_id,
      mii.quantity, mii.created_at,
      i.name AS inventory_name, i.unit,
      i.package_size, i.package_unit,
      i.stock_quantity, i.purchase_price,
...
async function addRecipeIngredient({
  menu_item_id, inventory_id, variant_id = null, quantity, restaurant_id
}) {
  const inventoryItem = await getItemById(inventory_id, restaurant_id);
  ...
  if (menuCheck.rows.length === 0) throw new Error("Menu item not found");

  let normalizedVariantId = null;

  if (variant_id !== null && variant_id !== undefined && String(variant_id).trim() !== "") {
    const variantCheck = await pool.query(
      `SELECT id FROM menu_item_variants WHERE id = $1 AND menu_item_id = $2`,
      [variant_id, menu_item_id]
    );
    if (variantCheck.rows.length === 0) throw new Error("Variant not found for this menu item");
  ...
  const result = await pool.query(
    `INSERT INTO menu_item_ingredients (menu_item_id, inventory_id, variant_id, quantity)
     VALUES ($1,$2,$3,$4) RETURNING *`,
    [menu_item_id, inventory_id, normalizedVariantId, quantity]
  );

  return result.rows[0];
}

async function updateRecipeIngredient(id, quantity, restaurantId) {
  const query = `
...
async function deleteRecipeIngredient(id, restaurantId) {
  const query = `
...
async function getDashboard(restaurantId) {
  const summaryQuery = `
    SELECT
      ...
      COUNT(*) FILTER (WHERE is_active = TRUE AND stock_quantity <= minimum_stock)::int AS low_stock_items,
      COUNT(*) FILTER (WHERE is_active = TRUE AND stock_quantity > minimum_stock)::int AS healthy_items,
      COALESCE(SUM(stock_quantity * purchase_price) FILTER (WHERE is_active = TRUE), 0) AS total_stock_value
    FROM inventory_items
    WHERE restaurant_id = $1
  `;

  const lowStockQuery = `
    SELECT id, name, category, unit, stock_quantity, minimum_stock, purchase_price, supplier
    FROM inventory_items
    WHERE restaurant_id = $1 AND is_active = TRUE AND stock_quantity <= minimum_stock
    ORDER BY stock_quantity ASC, name ASC
  `;

  const todayTransactionsQuery = `
    SELECT
      COALESCE(SUM(CASE WHEN type = 'IN' THEN quantity ELSE 0 END), 0) AS today_stock_in,
      COALESCE(SUM(CASE WHEN type = 'OUT' THEN quantity ELSE 0 END), 0) AS today_stock_out
    FROM inventory_transactions t
    INNER JOIN inventory_items i ON i.id = t.inventory_id
  ...
  const recentTransactionsQuery = `
    SELECT t.id, t.inventory_id, t.type, t.quantity, t.note, t.created_at, i.name, i.unit
    FROM inventory_transactions t
    INNER JOIN inventory_items i ON i.id = t.inventory_id
    WHERE i.restaurant_id = $1
    ORDER BY t.created_at DESC, t.id DESC
  ...
  const [summaryResult, lowStockResult, todayResult, recentResult] = await Promise.all([
